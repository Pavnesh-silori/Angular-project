...
export class AuthController {
    ...
    logout(redirectUrl, pageUrl, action, applicationKeyID = '') {
        ...
        if (applicationKeyID != ApplicationKeyID.ACCOUNT_KEY_ID && action == 'interceptor') {
            window.location.href = `${environment.ACCOUNT_URL}login?redirectUrl=${redirectUrl}&pageUrl=${pageUrl}`;
        } else {
            window.location.href = `${environment.ACCOUNT_URL}login?redirectUrl=${redirectUrl}&pageUrl=${pageUrl}&action=${action}`;
        }
    }

    

    getRolePermission(orgID, userID) {
        return this.http.get(`${environment.ORGANIZATION_API_URL}organizations/${orgID}/users/${userID}/roles`);
    }
