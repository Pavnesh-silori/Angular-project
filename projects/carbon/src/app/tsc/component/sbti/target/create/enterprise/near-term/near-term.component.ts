...
import { CalculationMethodEnum, EnterpriseEnum, NearTermEnum, OrgSizeEnum, SBTiTargetEnum, ScopeCoverageEnum, ScopeCoverageList, StatusEnum, StatusList } from '@carbon/enum/sbti.enum';
...
// / tsc-library
@Component({
    selector: 'app-near-term',
    templateUrl: './near-term.component.html',
    styleUrls: ['./near-term.component.scss']
})
export class NearTermComponent implements OnInit {
    ...
    minTotalReduction = NearTermEnum.MIN_TOTAL_REDUCTION_PERCENTAGE;
    ...
    constructor(
        ...
        private dialogRef: MatDialogRef<NearTermComponent>,
    ...
    ngAfterViewInit() {
        ...
        this.targetFG.get('submissionYear').valueChanges.subscribe((submissionYear) => {
            if (submissionYear != null) {
                ...
                if (!this.isUpdate) {
                    this.targetYearAC.patchValue(this.endYear);
                }
            }
        });
        ...
        this.scopeAC.valueChanges.subscribe((value) => {
            if (Array.isArray(value) && value.includes(3)) {
                this.showActivity = true;
                this.annualReductionTooltip = 'For Scope 3 targets, SBTi mandates a 2.5% minimum linear annual rate of reduction when modelling targets using the cross sector absolute reduction method.';
    ...
    targetFGInit() {
        this.targetFG = this.formBuilder.group({
            ...
            totalReductionPct: [, [Validators.required, Validators.max(PercentageEnum.MAX_PERCENTAGE)]],
    ...
    getTargetCreatedActivityList() {
        this.sbtiController.getTargetCreatedActivityList(this.currentOrgID, NearTermEnum.NEAR_TERM_REDUCTION).subscribe(response => {
    ...
    getMinimumReductionPctValue() {
        if (this.scopeAC.value.includes(3)) {
            this.totalReductionPctAC.setValidators([Validators.required, Validators.min(NearTermEnum.MIN_TOTAL_REDUCTION_PERCENTAGE_S3), Validators.max(PercentageEnum.MAX_PERCENTAGE)]);
            this.totalReductionPctAC.updateValueAndValidity({ emitEvent: false });
            this.minTotalReduction = NearTermEnum.MIN_TOTAL_REDUCTION_PERCENTAGE_S3;
        } else {
            this.totalReductionPctAC.setValidators([Validators.required, Validators.min(NearTermEnum.MIN_TOTAL_REDUCTION_PERCENTAGE), Validators.max(PercentageEnum.MAX_PERCENTAGE)]);
            this.totalReductionPctAC.updateValueAndValidity({ emitEvent: false });
            this.minTotalReduction = NearTermEnum.MIN_TOTAL_REDUCTION_PERCENTAGE;
        }
    }
