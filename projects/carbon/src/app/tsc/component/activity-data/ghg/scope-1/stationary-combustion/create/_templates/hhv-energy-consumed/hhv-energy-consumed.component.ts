...
import { InvalidForm, COMMON_CONSTANT, MatSelectSearchService, MaterialFormFieldAppearance, TypeConfirmationDialog, NocFileUploadEnum, FormErrorEnum } from '@library/tsc-common';
...
@Component({
    selector: 'stationary-combustion-hhv-energy-consumed',
    templateUrl: './hhv-energy-consumed.component.html',
    styleUrls: ['./hhv-energy-consumed.component.scss']
})
export class HhvEnergyConsumedComponent implements OnInit {
    ...
    patchFormData() {
        ...
        // if (this.activityData.standardFuelID != null) {
        //     let fuel = this.fuelUnitList.find(fuel => fuel['uID'] == 'S' + this.activityData.standardFuelID);
        //     console.log('fuelUnitList --',this.fuelUnitList);
        //     this.setStdFuelUnit(0, fuel['unitType']);
        // }
        // if (this.activityData.customFuelID != null) {
        //     this.setCustomFuelUnit(0);
        // }
    ...
    isFormFilled(formGroup) {
        ...
        if (formGroup.get('amountOfFuel').value != null) this.filled = true;
        if (formGroup.get('amountOfFuelUnitID').value != null) this.filled = true;
    ...
    addValidators(formGroup) {
        ...
        formGroup.get('amountOfFuel').addValidators([Validators.required, Validators.min(0)]);
        formGroup.get('amountOfFuelUnitID').addValidators([Validators.required]);
    ...
    removeValidators(formGroup) {
        ...
        formGroup.get('amountOfFuel').clearValidators();
        formGroup.get('amountOfFuelUnitID').clearValidators();
    ...
    updateValueAndValidity(formGroup) {
        ...
        formGroup.get('amountOfFuel').updateValueAndValidity({ emitEvent: false });
        formGroup.get('amountOfFuelUnitID').updateValueAndValidity({ emitEvent: false });
    ...
    async getFuel() {
        ...
        if (this.action == 'update') {
            filterFuel = stdFuel.filter(fuel => fuel.efSourceKeyID == this.activityData.emissionFactorDatabase.keyID);
        }
    ...
    isValidForm(): boolean {
        ...
        formArray.controls.forEach((control: FormGroup, index: number) => {
            if (control.get('amountOfFuel').value != null) {
    ...
    customeFuleSelected(fuelID, index) {
        this.customFuelController.getCustomFuelByID(this.currentOrgID, fuelID).subscribe(res => {
            this.setCustomNcv(index, res, fuelID);
        });

        this.activityDataFA.controls[index].get('amountOfFuelUnitID').reset(null, { emitEvent: false });

        this.setCustomFuelUnit(index);
    }

    setCustomFuelUnit(index) {
        // set unit dropDown
        this.unitIdList = [];
    ...
    selectSearch() {
        // ngx mat select search
        this.unitSearchUtil.entityArr = this.unitIdList;
