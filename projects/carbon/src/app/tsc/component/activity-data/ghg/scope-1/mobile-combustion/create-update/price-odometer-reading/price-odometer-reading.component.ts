...
@Component({
    selector: 'price-odometer-reading',
    templateUrl: './price-odometer-reading.component.html',
    styleUrls: ['./price-odometer-reading.component.scss']
})
export class PriceOdometerReadingComponent implements OnInit {
    ...
    updateFormControls() {
        this.activityDataDetailsFA.controls.forEach((formGroup: FormGroup) => {
            ...
            const milageNum = this.milageNum;
            const milageDenum = this.milageDenum;
            const currency = this.currency;

            if (currency) {
                formGroup.get('billCurrencyID').setValue(currency);
            }
            if (milageNum) {
                formGroup.get('mileageNumeratorUnitID').setValue(milageNum);
            }
            if (milageDenum) {
                formGroup.get('mileageDenominatorUnitID').setValue(milageDenum);
            }
        });
    }
    ...
    numeric_valid_fields: string[] = ['billAmount', 'fuelRate', 'openingOdometerReading', 'closingOdometerReading', 'mileage'];
    valid_fields: string[] = ['dateRange', 'fuelTypeID', 'billAmount', 'billCurrencyID', 'fuelRate', 'fuelRateNumeratorUnitID', 'fuelRateDenominatorUnitID', 'openingOdometerReading', 'closingOdometerReading', 'odometerReadingUnitID', 'mileage', 'mileageNumeratorUnitID', 'mileageDenominatorUnitID'];
    isFormFilled(formGroup) {
        for (let control of this.valid_fields) {
            if (!this.filled) {
                this.filled = (formGroup.get(control).value != null);
            }
        }
    }
    ...
    addValidators(formGroup) {
        for (let control of this.valid_fields) {
            ...
            if (this.numeric_valid_fields.includes(control)) {
                validators.push(Validators.min(0));
            }
            formGroup.get(control).addValidators(validators);
        }
        ...
        let to_clear_validators: string[];
        const bill_controls: string[] = ['billAmount', 'billCurrencyID'];
        const fuel_rate_controls: string[] = ['fuelRate', 'fuelRateNumeratorUnitID', 'fuelRateDenominatorUnitID'];
        const mileage_controls: string[] = ['mileage', 'mileageNumeratorUnitID', 'mileageDenominatorUnitID'];
        const meter_reading_controls: string[] = ['openingOdometerReading', 'closingOdometerReading', 'odometerReadingUnitID'];

        if (fuelRate != null && billAmount != null) {
            if (openingOdometer != null && closingOdometer != null) {
                to_clear_validators = mileage_controls;
            } else if (mileage != null) {
                to_clear_validators = meter_reading_controls;
            } else {
                to_clear_validators = [];
            }
        } else if ((openingOdometer != null && closingOdometer != null) && mileage != null) {
            to_clear_validators = [...bill_controls, ...fuel_rate_controls];
        } else {
            to_clear_validators = [];
        }

        for (let control of to_clear_validators) {
