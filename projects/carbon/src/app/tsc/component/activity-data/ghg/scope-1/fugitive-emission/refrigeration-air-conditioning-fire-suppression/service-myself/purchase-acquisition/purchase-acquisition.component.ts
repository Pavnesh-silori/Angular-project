...
@Component({
	selector: 'app-purchase-acquisition',
	templateUrl: './purchase-acquisition.component.html',
	styleUrls: ['./purchase-acquisition.component.scss']
})
export class PurchaseAcquisitionComponent implements OnInit {
	...
	purchaseTypeFlag: boolean = false;
	purchaseTypeFC: FormControl = new FormControl('all');
	...
	titleName: string = 'purchase and acquisition record';
	activityType: string = EquipmentTabEnum.PURCHASE;
	calculationApproachKeyID = CalculationApproachKeyIDEnum.CALCULATION_SERVICE_MYSELF;
	...
	@ViewChild('purchaseTypeSelect') purchaseTypeSelect: FunnelFilterComponent;
		...
		merge(
			...
			.pipe(
				...
				switchMap(() => {
					...
					if (this.purchaseTypeFlag) {
						this.filterArr.push('activityTypeID:' + this.purchaseTypeFC.value);
					}
	...
	createPurchaseAcquisitionRecord(operation) {
		let dialogRef;
		if (operation == 'CREATE') {
	...
	updatePurchaseAcquisitionRecord(operation, equipmentCapacity) {
		let dialogRef;
		if (operation == 'UPDATE') {
			dialogRef = this.dialog.open(MyselfPurchaseSaleRetrofitDialog, {
				data: {
	...
	confirmDelete(equipmentCapacity) {
        ...
        const dialog = this.dialog.open(DialogOneComponent,
			{
                data: {
                    type: 'delete',
                    icon: 'dangerous',
                    header: 'Delete myself purchase sale retrofit record?',
		...
		dialog.afterClosed().subscribe(res => {
            if(res){
				this.deletePurchaseAcquisitionRecord(equipmentCapacity['id']);
            }
		});
	}

	deletePurchaseAcquisitionRecord(equipmentCapacityID) {
		this.fugitiveMyselfManufacturerController.deleteEquipmentGasQuantityRecord(this.currentOrgID, equipmentCapacityID, this.calculationApproachKeyID).subscribe((deleteRes) => {
			...
			error => {
				console.log('Error in deletePurchaseAcquisitionRecord -', error);
			})
	}
	...
	async getPurchaseTypeList() {
		this.purchaseTypeList = <any>await this.fugitiveActivityTypeService.getFugitiveActivityType(this.activityType, this.calculationApproachKeyID);
	}
	...
	async getActivityConfiguration() {
		...
		this.showAddRecordButton = this.responseM['createServiceMyselfApproach'];
	}
}
