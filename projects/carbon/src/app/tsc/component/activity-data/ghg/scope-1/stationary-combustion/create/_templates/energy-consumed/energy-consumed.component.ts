...
import { IpccTechnologyParameterService } from '@carbon/service/ipcc-technology-parameter.service';
...
// / tsc-library
@Component({
    selector: 'stationary-combustion-energy-consumed',
    templateUrl: './energy-consumed.component.html',
    styles: [
    ]
})
export class EnergyConsumedComponent implements OnInit {
    ...
    parameterSearchUtil: MatSelectSearchService = new MatSelectSearchService(['name']);
    ...
    constructor(
        ...
        private ipccTechnologyParameterService: IpccTechnologyParameterService,
    ...
    isFormFilled(formGroup) {
        ...
        if (formGroup.get('amountOfEnergy').value != null) this.filled = true;
        if (formGroup.get('amountOfEnergyUnitID').value != null) this.filled = true;
    }
    ...
    addValidators(formGroup) {
        ...
        formGroup.get('amountOfEnergy').addValidators([Validators.required, Validators.min(0)]);
        formGroup.get('amountOfEnergyUnitID').addValidators([Validators.required]);
    ...
    removeValidators(formGroup) {
        ...
        formGroup.get('amountOfEnergy').clearValidators();
        formGroup.get('amountOfEnergyUnitID').clearValidators();
    ...
    updateValueAndValidity(formGroup) {
        ...
        formGroup.get('amountOfEnergy').updateValueAndValidity({ emitEvent: false });
        formGroup.get('amountOfEnergyUnitID').updateValueAndValidity({ emitEvent: false });
    ...
    isValidForm(): boolean {
        ...
        formArray.controls.forEach((control: FormGroup, index: number) => {
            if (control.get('amountOfEnergy').value != null) {
    ...
    async getTechnologyList() {
        this.technologyList = await this.ipccTechnologyParameterService.getIpccTechnologyParameterList();
        ...
        if (this.technologyList.length > 0) {
            ...
            this.technologyList.push(metric);
        }
    ...
    getParameter(id) {
        let technology = this.technologyList.find(item => item.id == id);
        if (technology) {
            this.parameterList = technology.parameter;

            this.parameterSearchUtil.entityArr = this.parameterList;
