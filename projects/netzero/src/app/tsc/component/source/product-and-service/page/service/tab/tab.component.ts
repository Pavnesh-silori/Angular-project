...
import { TypeConfirmationDialog, DialogOneComponent } from '@library/tsc-common';
...
export class TabComponent implements OnInit {
	...
	close() {
		this.router.navigate(['/source/product-service/service/page']);
	}

	removeItem() {
		const selectedRows = [this.productID];
		this.showDeleteConfirmation(selectedRows);
	}


	showDeleteConfirmation(items: any[]) {
		const name = this.serviceViewData[0]?.name;
		// const message = `Are you sure you want to delete these items?`;
		const message = `Are you sure you want to mark ${name}`;
	...
	changeItemStatus() {
		const status = this.serviceViewData.status.toLowerCase() === 'active' ? 'inactive' : 'active';
		this.showUpdateConfirmation(status);
	}

	showUpdateConfirmation(status: string) {
		const name = this.serviceViewData?.name ? this.serviceViewData?.name : 'service';
		const newStatus = status.toLowerCase();
		const message = `Are you sure you want to mark ${name} as ${newStatus}?`;
		let body = '';
		if (newStatus === 'active') {
	...
	updateStatusForItems(status: string) {
		let data = [];
		data.push(this.serviceViewData.id);
		this.productAndServiceController.updateProductStatus(this.currentOrgID, data, status, ProductServiceTypeEnum.SERVICE_KEY_ID).subscribe((updateRes: any) => {
	...
	updateViewPage() {
		this.productAndServiceController.getServiceByID(this.currentOrgID, this.productID).subscribe((res: any) => {
			this.serviceViewData = res;
			this.router.navigateByUrl('/dummy', { skipLocationChange: true }).then(() => {
				this.router.navigate(['/source/product-service/service', this.productID, 'details']);
			});
		})
	}
	...
	deleteItems() {
		...
		const formData = { recordID: deletedItem.map(item => item.activityDataID) };
